...
integration_test: $(EXECUTABLE)
	...
	@./$(EXECUTABLE) 7 -p 2>&1 | grep -q "prime number" && echo "SUCCESS: Correct prime detection" || echo "ERROR: Incorrect prime detection"
	@echo "Subtest 5.2: Composite number"
	@./$(EXECUTABLE) 10 -p 2>&1 | grep -q "composite number" && echo "SUCCESS: Correct composite detection" || echo "ERROR: Incorrect composite detection"
	...
	@echo "Subtest 9.1: Division by zero"
	@./$(EXECUTABLE) 0 -h 2>&1 | grep -q "division by zero" && echo "SUCCESS: Correct division by zero handling" || echo "ERROR: Incorrect division by zero handling"
	@echo "Subtest 9.2: Invalid number for -a"
	@./$(EXECUTABLE) -5 -a 2>&1 | grep -q "wrong number" && echo "SUCCESS: Correct invalid number handling" || echo "ERROR: Incorrect invalid number handling"
	@echo "Subtest 9.3: Invalid number for -e"
	@./$(EXECUTABLE) 11 -e 2>&1 | grep -q "wrong number" && echo "SUCCESS: Correct invalid number handling" || echo "ERROR: Incorrect invalid number handling"
